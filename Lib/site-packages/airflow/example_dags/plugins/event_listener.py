...
def on_task_instance_running(previous_state: TaskInstanceState, task_instance: TaskInstance, session):
    ...
    print(f"dags name:{dag_name} and current dag run status:{dagrun_status}")
...
def on_task_instance_success(previous_state: TaskInstanceState, task_instance: TaskInstance, session):
    ...
    print(f"dags name:{dag_id} queued_at:{queued_at}")
...
def on_dag_run_success(dag_run: DagRun, msg: str):
    ...
    """
    print("dags run in success state")
    start_date = dag_run.start_date
    end_date = dag_run.end_date

    print(f"dags run start:{start_date} end:{end_date}")
...
def on_dag_run_failed(dag_run: DagRun, msg: str):
    ...
    """
    print("dags run  in failure state")
    dag_id = dag_run.dag_id
    run_id = dag_run.run_id
    external_trigger = dag_run.external_trigger

    print(f"dags information:{dag_id} Run id: {run_id} external trigger: {external_trigger}")
...
def on_dag_run_running(dag_run: DagRun, msg: str):
    ...
    """
    print("dags run  in running state")
    queued_at = dag_run.queued_at
    dag_hash_info = dag_run.dag_hash

    print(f"dags information Queued at: {queued_at} hash info: {dag_hash_info}")
