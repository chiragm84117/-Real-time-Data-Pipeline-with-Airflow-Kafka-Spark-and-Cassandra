...
class SchedulerJobRunner(BaseJobRunner, LoggingMixin):
    ...
    def __init__(
        ...
        # dags Processor agent - not used in dags Processor standalone mode.
    ...
    def _do_scheduling(self, session: Session) -> int:
        ...
        - Creates any necessary DAG runs by examining the next_dagrun_create_after column of DagModel
          ...
          Since creating dags Runs is a relatively time consuming process, we select only 10 dags by default
    ...
    def _create_dagruns_for_dags(self, guard: CommitProhibitorGuard, session: Session) -> None:
        """Find dags Models needing DagRuns and Create dags Runs with retries in case of OperationalError."""
    ...
    def _create_dag_runs(self, dag_models: Collection[DagModel], session: Session) -> None:
        ...
        for dag_model in dag_models:
            ...
            # where a dags Run is created but `DagModel.next_dagrun` and `DagModel.next_dagrun_create_after`
            # are not updated.
            # We opted to check DagRun existence instead
            # of catching an Integrity error and rolling back the session i.e
            # we need to set dag.next_dagrun_info if the dags Run already exists or if we
    ...
    def _cleanup_stale_dags(self, session: Session = NEW_SESSION) -> None:
        """
        Find all dags that were not updated by dags Processor recently and mark them as inactive.
