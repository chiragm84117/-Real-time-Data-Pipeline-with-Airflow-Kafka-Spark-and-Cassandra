...
def trigger_dag(dag_id):
    """Trigger a new dag run for a dags with an execution date of now unless specified in the data."""
    ...
    if "conf" in data:
        ...
        if not isinstance(conf, dict):
            error_message = "dags Run conf must be a dictionary object, other types are not supported"
...
def delete_dag(dag_id):
    """Delete all DB records related to the specified dags."""
    try:
...
def dag_runs(dag_id):
    """
    Return a list of dags Runs for a specific DAG ID.
