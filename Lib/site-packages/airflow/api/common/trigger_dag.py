...
def _trigger_dag(
    ...
    if dag is None or dag_id not in dag_bag.dags:
        raise DagNotFound(f"dags id {dag_id} not found")
...
def trigger_dag(
    ...
    :param execution_date: date of execution
    :param replace_microseconds: whether microseconds should be zeroed
    :return: first dag run triggered - even if more than one dags Runs were triggered or None
    """
    dag_model = DagModel.get_current(dag_id)
    if dag_model is None:
        raise DagNotFound(f"dags id {dag_id} not found in DagModel")
