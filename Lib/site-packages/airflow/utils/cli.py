...
def get_dag_by_file_location(dag_id: str):
    ...
    if dag_model is None:
        raise AirflowException(
            f"dags {dag_id!r} could not be found; either it does not exist or it failed to parse."
        )
...
def get_dag(subdir: str | None, dag_id: str, from_db: bool = False) -> DAG:
    ...
    if not dag:
        if from_db:
            raise AirflowException(f"dags {dag_id!r} could not be found in DagBag read from database.")
        fallback_path = _search_for_dag_file(subdir) or settings.DAGS_FOLDER
        logger.warning("dags %r not found in path %s; trying path %s", dag_id, first_path, fallback_path)
